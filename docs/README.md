# feishu-bot-sdk 文档（中文）

[English](./README_EN.md)

本目录是 SDK 的模块化使用文档。每个模块都有独立页面，便于按需查阅和复制代码。

## 目录

- [01 核心客户端与配置](./zh/01-core-client.md)
- [02 IM 消息、群组与媒体](./zh/02-im.md)
- [03 Drive 文件与权限](./zh/03-drive.md)
- [04 多维表格（Bitable）](./zh/04-bitable.md)
- [05 云文档（Docx/Docs Content）](./zh/05-docx-and-docs.md)
- [06 Wiki 知识库](./zh/06-wiki.md)
- [07 事件系统（Events/Webhook/WS）](./zh/07-events-webhook-ws.md)
- [08 FeishuBotServer 长连接服务](./zh/08-bot-server.md)
- [09 类型、异常与限流](./zh/09-types-errors-rate-limit.md)
- [10 CLI 命令行工具](./zh/10-cli.md)
- [11 日历（Calendar）](./zh/11-calendar.md)
- [12 通讯录（Contact）](./zh/12-contact.md)
- [13 搜索（Search）](./zh/13-search.md)
- [14 云文档工作流速查](./zh/14-cloud-doc-workflows.md)

## 阅读建议

- 先看 [01 核心客户端与配置](./zh/01-core-client.md) 完成初始化。
- 如果你做机器人对话、群组管理、群公告、群菜单或群标签页，优先看 [02 IM 消息、群组与媒体](./zh/02-im.md) + [08 FeishuBotServer 长连接服务](./zh/08-bot-server.md)。
- 如果你做结果交付，重点看 [03 Drive 文件与权限](./zh/03-drive.md)、[04 多维表格](./zh/04-bitable.md)、[05 云文档](./zh/05-docx-and-docs.md)。
- 如果你要做“创建文档、局部修改、图片/附件替换、版本管理”这类整套流程，直接看 [14 云文档工作流速查](./zh/14-cloud-doc-workflows.md)。
- 如果你做日程协同，重点看 [11 日历（Calendar）](./zh/11-calendar.md)。
- 如果你做组织架构或成员查询，重点看 [12 通讯录（Contact）](./zh/12-contact.md)。
- 如果你要做应用/消息/文档检索，重点看 [13 搜索（Search）](./zh/13-search.md)。
